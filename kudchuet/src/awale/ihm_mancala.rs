	...
	fn ui(&mut self, ui: &mut egui::Ui, _frame: &mut eframe::Frame) {
		egui::CentralPanel::default().show_inside(ui, |ui| {
			...
			ui.horizontal(|ui| {
				...
				if Mancala::get_winner(&self.game).is_some() {
					ui.colored_label(
						egui::Color32::RED,
						"Partie terminée"
					);
				...
				if let Some(pos) = response.interact_pointer_pos() {
					if response.clicked() {
						if self.game.skipped {
							self.game=Mancala::apply(&mut self.game, 0).unwrap();
							return;
						}
						for i in 0..12 {
							let is_top = i >= 6;
							...
							if pit_rect.contains(pos) {
								...
								if Mancala::get_winner(&self.game).is_none() && self.game.to_move == is_top {
									let mv = if is_top {12-i} else {6-i} as u8;
									self.game=Mancala::apply(&mut self.game, mv).unwrap();
			...
			} else if Mancala::get_winner(&self.game).is_none() {
