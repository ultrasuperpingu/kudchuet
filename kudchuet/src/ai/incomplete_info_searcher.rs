...
where
	G: BoardGame,
	G::M: BoardMove<G>,
	E: Evaluator<G = G> + Default + Clone + Send + Sync + Eq + 'static,
{
	name: String,
	evaluator: E,
	initial_depth: u8,
...
where
	G: BoardGame,
	G::M: BoardMove<G>,
	T: Evaluator<G = G> + Default + Clone + Send + Sync + Eq + 'static,
{
...
where
	...
	E: Evaluator<G = G> + Default + Clone + Send + Sync + Eq + 'static + Debug,
{
	...
	fn build_engine(&self) -> Self::Engine {
		...
		let mut ai = IterativeSearch::new(self.evaluator.clone(), IterativeOptions::new());
