...
use super::table::*;
use super::util::*;
use super::{IterativeOptions, Stats};
...
struct ParallelExpectiMinimax<E: Evaluator> {
...
impl<E: Evaluator> ParallelExpectiMinimax<E>
where
	...
	E: Clone + Sync + Send + 'static,
{
		...
		E: Sync,
	{
		...
		self.countermoves
			.local_do(|cm| cm.reorder(prev_move, &mut moves));
...
impl<E: Evaluator> ParallelSearch<E> {
	...
	fn pretty_stats(
		...
		stats: &Stats,
		start: Instant,
		minimax: &ParallelExpectiMinimax<E>,
		depth: u8,
	) -> String {
	...
	E: Clone + Sync + Send + 'static,
{
	fn choose_move(&mut self, s: &<E::G as Game>::S) -> Option<<E::G as Game>::M> {
		...
		if self.par_opts.background_pondering {
			...
			let minimaxer = ParallelExpectiMinimax::new(
				self.opts,
				self.par_opts,
				self.eval.clone(),
				self.table.clone(),
			...
			self.thread_pool.spawn(move || {
				minimaxer.iterative_search(state, 99, player_to_move, true);
			});
		}
		Some(best_move)
	}
