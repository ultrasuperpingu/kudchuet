...
pub(super) struct ExpectiMinimaxer<E: Evaluator, T> {
	#[cfg(not(target_arch = "wasm32"))]
...
impl<E: Evaluator, T: Table<<E::G as Game>::M>> ExpectiMinimaxer<E, T>
where
	<E::G as Game>::M: Copy + Eq,
{
	...
	fn null_move_check(
			...
			(self.opts.null_move_depth, E::G::null_move(s))
		{
			...
			if depth > depth_reduction &&
                ...
                self.eval.evaluate_for(s, player_to_move) >= beta
			{
	...
	// Recursively compute minimax on the game state. Returns None if it hits the timeout or search is cancelled.
	pub(super) fn expectiminimax(
		&mut self,
		s: &mut <E::G as Game>::S,
		prev_move: Option<<E::G as Game>::M>,
		mut depth: u8,
		player_to_move: Player,
		mut alpha: Evaluation,
		mut beta: Evaluation,
		...
		if let Some(value) = self
			.table
			.check(hash, depth, &mut good_move, &mut alpha, &mut beta)
		{
		...
		if need_store {
			//TODO: check if we need to store a random
			self.table
				.update(hash, alpha_orig, beta, depth, best, best_move);
		}
		self.move_pool.free(moves);
	...
	pub(super) fn search_and_reorder(
		&mut self,
		s: &mut <E::G as Game>::S,
		moves: &mut [ValueMove<<E::G as Game>::M>],
		depth: u8,
		player_to_move: Player,
	) -> Option<Evaluation> {
	...
	<E::G as Game>::M: Copy + Eq,
{
	fn choose_move(&mut self, s: &<E::G as Game>::S) -> Option<<E::G as Game>::M> {
		...
		self.minimaxer
			.countermoves
			.advance_generation(E::G::null_move(s));
		// Reset stats.
		...
		while depth <= self.max_depth {
			...
			let search = {
				...
				self.minimaxer.search_and_reorder(
					&mut s_clone,
					&mut moves[..],
					depth,
					player_to_move,
			...
			self.nodes_explored
				.push(self.minimaxer.stats.nodes_explored);
			self.minimaxer.stats.nodes_explored = 0;
			self.prev_value = entry.value;
			depth += self.opts.step_increment;
			self.minimaxer
				.table
				.populate_pv::<E::G>(&mut self.pv, &s_clone);
