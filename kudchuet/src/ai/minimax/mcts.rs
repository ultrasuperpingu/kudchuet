...
use super::SearchStopSignal;

use super::common::{move_id, pv_string, random_best};
use super::interface::*;
use super::sync_util::*;
use super::util::AppliedMove;
...
impl<M> Node<M> {
	...
	fn uct_score(
		...
		let exploitation = if current_player == player_to_eval {
			win_ratio
		} else {
			1.0 - win_ratio
		};

		exploitation + exploration_score * (2.0 * log_parent_visits / visits).sqrt()
		...
		loop {
			...
			let m = match self.random_move(&mut state, &mut moves, &mut rng) {
				Err(winner) => {
					...
					return match winner {
						Winner::Player(p) => {
							if p as usize == player_to_eval.idx() {
								if first { WIN } else { 1 }
							} else {
								if first { LOSS } else { -1 }
							}
						}
						Winner::Draw => 0,
					};
...
impl<G: Game> MonteCarloTreeSearch<G> {
	...
	pub fn new_with_policy(
		options: MCTSOptions,
		policy: Box<dyn RolloutPolicy<G = G> + Send + Sync>,
	) -> Self {
		Self {
			options,
			max_rollouts: 0,
	...
	fn rollout(&self, state: &G::S, player_to_eval: Player) -> i32
	where
		G: Sync,
		G::S: Clone,
	{
		self.rollout_policy
			.as_ref()
			.map(|p: &Box<dyn RolloutPolicy<G = G> + Send + Sync>| {
				p.rollout(&self.options, state, player_to_eval)
			})
			.unwrap_or_else(|| {
				DumbRolloutPolicy::<G> {
					game_type: PhantomData,
				}
				.rollout(&self.options, state, player_to_eval)
			})
	}
		...
		G::S: Clone,
	{
		...
		if force_rollout {
			return node.update_stats(self.rollout(state, player_to_eval));
		}
			...
			None => {
				...
				if node.visits.load(SeqCst) <= self.options.rollouts_before_expanding {
					...
					match new_expansion::<G>(state) {
						Err(Winner::Player(p)) => {
							if p as usize == player_to_eval.idx() {
								return node.update_stats(WIN);
							} else {
								return node.update_stats(LOSS);
							}
						}
		...
		// Recurse.
		let next = match node.best_child(1., current_player, player_to_eval) {
			Some(child) => child,
		...
		let child_result = self.simulate(next, &mut new, force_rollout, player_to_eval)?;

		// Propagate up forced wins and losses.
		let result = if child_result == WIN {
			// Having a guaranteed win child makes you a win parent.
			WIN
		} else if child_result == LOSS {
			// Having all guaranteed loser children makes you a looser parent.
			if expansion
				.children
				.iter()
	...
	G::M: Copy + Sync,
{
	fn choose_move(&mut self, s: &G::S) -> Option<G::M> {
		...
		let (rollouts_per_thread, extra) = if self.max_rollouts == 0 {
			...
			(
				rollouts_per_thread,
				self.max_rollouts - rollouts_per_thread * num_threads,
			)
		};
		...
		thread::scope(|scope| {
			for i in 0..num_threads {
				...
				scope.spawn(move || {
					...
					for _ in 0..rollouts {
						if mtcs
							.simulate(node, &mut state, false, player_to_eval)
							.is_none()
						{
							break;
						}
		...
		self.pv.clear();
		let mut node = &*root;
		while let Some(best) = node.best_child(0.0, current_player, player_to_eval) {
		...
		root.best_child(exploration, current_player, player_to_eval).map(|node| node.m.unwrap())
	}
