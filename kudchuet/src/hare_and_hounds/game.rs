...
impl minimax::Evaluator for HareAndHoundsEval {
	...
	fn evaluate(&self, state: &HareAndHounds) -> minimax::Evaluation {
		...
		let hare_moves = (super::NEIGHBORS_HARE[hare as usize] & !state.houds).count() as i16;
...
mod tests {
	...
	use crate::hare_and_hounds::game::HareAndHoundsEval;
	...
	fn legal_moves(state: &HareAndHounds) -> Vec<HareAndHounds> {
		...
		let mut moves= [crate::hare_and_hounds::Move::default(); HareAndHounds::MAX_MOVES];
		let mut len=0;
		children.reserve(9);
