...
use crate::common::{GameResult, Player, bitboards::Bitboard9x13};
...
impl Bitboard9x13 {
	const fn compute_shoot_mask(sq:u8) -> Self {
	...
	const fn generate_pass_mask_table() -> [Self; Self::NB_SQUARES] {
	...
	const fn all_lines_at_south(sq:u8) -> Self {
		let mut i=0;
	...
	const fn all_lines_at_north(sq:u8) -> Self {
	...
	const fn compute_black_tackle_position_mask(sq:u8) -> Self {
		let index = sq as usize;
		let mask = Self::NEIGHBORS_8[index].storage() & !Self::all_lines_at_south(sq).storage();
		Self::from_storage(mask)
	}
	const fn compute_white_tackle_position_mask(sq:u8) -> Self {
		let index = sq as usize;
		let mask = Self::NEIGHBORS_8[index].storage() & !Self::all_lines_at_north(sq).storage();
		Self::from_storage(mask)
	}
	const fn generate_black_tackle_position_table() -> [Self; Self::NB_SQUARES] {
		let mut arr = [Self(0); Self::NB_SQUARES];
		let mut i = 0;
		while i < Self::NB_SQUARES {
	...
	const fn generate_white_tackle_position_table() -> [Self; Self::NB_SQUARES] {
		let mut arr = [Self(0); Self::NB_SQUARES];
		let mut i = 0;
		while i < Self::NB_SQUARES {
...
pub struct Move ([Option<Action>;ACTION_PER_MOVE]);

#[derive(Clone, Copy, PartialEq, Eq, Debug, Hash)]
pub struct FootBoard {
	player1: Bitboard9x13,
	player2: Bitboard9x13,
	ball: u8,
	turn: u8,
	score1:u8,
	score2:u8,
...
#[cfg(test)]
mod tests {
	use crate::{common::bitboards::Bitboard9x13, footboard::FootBoard};
	#[test]
	fn test_bitboard() {
