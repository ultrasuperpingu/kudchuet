...
impl Display for Board {
	fn fmt(&self, f: &mut Formatter) -> Result {
		let red_pieces = if self.reds_move() {
			self.pieces_to_move
		} else {
			self.pieces_just_moved()
		};
		let yellow_pieces = if self.reds_move() {
			self.pieces_just_moved()
		} else {
			self.pieces_to_move
		};
...
impl Evaluator for BasicEvaluator {
	...
	fn evaluate_for(&self, b: &Board, p: Player) -> Evaluation {
		...
		for col in 2..5 {
			score += ((player_pieces >> (HEIGHT * col)) & COL_MASK).count_ones() as Evaluation;
			score -= ((opponent_pieces >> (HEIGHT * col)) & COL_MASK).count_ones() as Evaluation;
		}
...
fn main() {
	...
	if std::env::args().any(|arg| arg == "perft") {
		perft::<Connect4Game>(&mut b, 10, false);
		return;
	}
	...
	let mut iterative = IterativeSearch::new(
		BasicEvaluator::default(),
		opts.clone().with_aspiration_window(5),
	);
	...
	#[cfg(not(target_arch = "wasm32"))]
	let mut strategies: [&mut dyn Strategy<Connect4Game>; 3] =
	...
	while Connect4Game::get_winner(&b).is_none() {
		...
		match strategy.choose_move(&mut b) {
			Some(m) => {
				...
				b = Connect4Game::apply(&mut b, m).unwrap();
			}
			None => break,
		}
		s = 1 - s;
