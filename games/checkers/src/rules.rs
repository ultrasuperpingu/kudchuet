...
impl Default for Checkers10 {
	fn default() -> Self {
		...
		let pawns_per_player =
			Bitboard5x10Checkers10::WIDTH * (Bitboard5x10Checkers10::HEIGHT / 2 - 1);
...
impl Display for Checkers10 {
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		...
		write!(f, "  ")?;
		for c in Bitboard5x10Checkers10::WIDTH * (Bitboard5x10Checkers10::HEIGHT - 1)
			..Bitboard5x10Checkers10::WIDTH * (Bitboard5x10Checkers10::HEIGHT - 1)
				+ Bitboard5x10Checkers10::WIDTH
		{
			write!(f, " {:2} ", c + 1)?;
		}
		writeln!(f)
...
impl Checkers10 {
	...
	pub fn play_unchecked(&mut self, m: &Move) {
		debug_assert!(
			self.white_pawns.0 & self.black_pawns.0 & self.white_queens.0 & self.black_queens.0
				== 0
		);
	...
	fn pawn_takes_moves(
		...
		let take_count = Self::pawn_one_take_moves(
			...
			opponent_pawns,
			opponent_queens,
			if self.rules.pawns_takes_backward {
				0..4
			} else if advance_way < 0 {
				0..2
			} else {
				2..4
			},
	...
	fn pawn_one_take_moves(
		...
		for i in range {
			let jump = jumps[i];
			if jump.1 == Bitboard5x10Checkers10::NB_SQUARES as u8 {
				continue;
			}
			if ((jump.0 & all_pieces).is_empty() || start_cell_index == jump.1)
				&& !already_taken.get_at_index(jump.2 as usize)
			{
				...
				if is_queen || is_pawn {
					takes_out[nb_takes] = Move {
						start: cell_index,
						dest: jump.1,
						pawn_takes: if is_pawn {
							Bitboard5x10Checkers10::from_index(jump.2 as usize)
						} else {
							Bitboard5x10Checkers10::EMPTY
						},
						queen_takes: if is_queen {
							Bitboard5x10Checkers10::from_index(jump.2 as usize)
						} else {
							Bitboard5x10Checkers10::EMPTY
						},
	...
	fn queen_simple_moves_long(
		...
		// On all 4 directions
		for ray in [
			Bitboard5x10Checkers10::RAY_NE[sq],
			Bitboard5x10Checkers10::RAY_SE[sq],
			Bitboard5x10Checkers10::RAY_NW[sq],
			Bitboard5x10Checkers10::RAY_SW[sq],
		] {
			...
			let landing_mask = if blockers.is_empty() {
				...
				let is_forward = ray == Bitboard5x10Checkers10::RAY_NE[sq]
					|| ray == Bitboard5x10Checkers10::RAY_SE[sq];
	...
	fn queen_simple_moves_short(
		...
		for dest in
			(Bitboard5x10Checkers10::QUEEN_NEIGHBORS[cell_index as usize] & !all_pieces).iter_bits()
		{
			moves.push(Move {
				start: cell_index,
				dest: dest as u8,
	...
	fn queen_one_take_moves(
		...
		for (ray_table, is_decreasing_ray) in directions {
			...
			if !(victim & opponent_all).is_empty() && (victim & already_taken).is_empty() {
				...
				let obstacles_behind =
					ray_from_victim
						& all_pieces & !Bitboard5x10Checkers10::from_index(start_cell_index as usize);
				...
				for dest_idx in landing_mask.iter_bits() {
					...
					let (pawn_takes, queen_takes) =
						if opponent_queens.get_at_index(first_blocker_idx) {
							(
								Bitboard5x10Checkers10::EMPTY,
								Bitboard5x10Checkers10::from_index(first_blocker_idx),
							)
						} else {
							(
								Bitboard5x10Checkers10::from_index(first_blocker_idx),
	...
	fn filter_moves(&self, moves: &mut Vec<Move>) {
		...
		if has_take {
			...
			if self.rules.maximal_take_mandatory {
				let max_len = moves.iter().map(|m| m.nb_takes()).max().unwrap_or(0);
...
mod tests {
	...
	fn test_checkers2() {
		let mut board = Checkers10 {
			...
			white_pawns: Bitboard5x10Checkers10(
				0b00100_00000_00000_00000_00000_00000_00000_00000_00000_00000u64,
			),
			black_pawns: Bitboard5x10Checkers10(
				0b00000_00100_00000_01000_00000_01110_00000_01110_00000_00000u64,
			),
			white_queens: Bitboard5x10Checkers10(0),
			black_queens: Bitboard5x10Checkers10(0),
			..Default::default()
	...
	fn test_checkers3() {
		let mut board = Checkers10 {
			...
			black_pawns: Bitboard5x10Checkers10(
				0b00000_00011_00000_00000_00100_01000_00110_00000_01000_00000u64,
			),
			white_queens: Bitboard5x10Checkers10(
				0b00000_00000_00100_00000_00000_00000_00000_00000_00000_00000u64,
			),
			black_queens: Bitboard5x10Checkers10(0),
			..Default::default()
		};
