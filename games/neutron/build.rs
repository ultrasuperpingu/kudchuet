...
fn generate_neutron_pext_tables() {
	...
	for (i, item) in slide_masks
		.iter_mut()
		.enumerate()
		.take(Bitboard5x5::NB_SQUARES)
	{
		*item = slide_mask(i as u8);
	}

	let slide_pext = Bitboard5x5::generate_attack_tables_pext(slide_mask, |square, blockers| {
		Bitboard5x5::compute_slides(SLIDE_OFFSETS, square, blockers)
	});

	// Génération du code Rust
	writeln!(
		f,
		"pub static SLIDE_MASKS: [Bitboard5x5; {}] = {:?};",
		Bitboard5x5::NB_SQUARES,
		slide_masks
	)
	.unwrap();
	...
	writeln!(
		f,
		"pub static SLIDE_MOVES: [&[Bitboard5x5]; {}] = [",
		Bitboard5x5::NB_SQUARES
	)
	.unwrap();
