...
pub fn rook_mask(square: u8) -> Bitboard8x8 {
	Bitboard8x8::compute_row_mask(square as usize)
		& !Bitboard8x8::WEST_BORDER
		& !Bitboard8x8::EAST_BORDER
		| Bitboard8x8::compute_col_mask(square as usize)
			& !Bitboard8x8::NORTH_BORDER
			& !Bitboard8x8::SOUTH_BORDER
}
#[inline]
pub fn bishop_mask(square: u8) -> Bitboard8x8 {
	(Bitboard8x8::compute_diag_inc_mask(square as usize)
		| Bitboard8x8::compute_diag_dec_mask(square as usize))
		& !Bitboard8x8::BORDER
}

#[inline]
pub fn slide_mask(square: u8) -> Bitboard5x5 {
	Bitboard5x5::compute_diag_inc_mask(square as usize)
		| Bitboard5x5::compute_diag_dec_mask(square as usize)
		| Bitboard5x5::compute_col_mask(square as usize)
		| Bitboard5x5::compute_row_mask(square as usize)
}
impl Bitboard8x8
where
	<Self as Bitboard>::Storage: PrimInt + Unsigned,
{
	pub fn compute_attacks(offsets: &[(i8, i8)], square: u8, blockers: Self) -> Self {
		...
		Self::from_storage(<<Self as Bitboard>::Storage as IntegerStorage>::from_u128(
			attacks,
		))
	}
}
...
fn generate_chess_pext_tables() {
	const ROOK_OFFSETS: &[(i8, i8)] = &[(1, 0), (0, 1), (-1, 0), (0, -1)];
	const BISHOP_OFFSETS: &[(i8, i8)] = &[(1, 1), (-1, 1), (-1, -1), (1, -1)];
	...
	for (i, mask) in rook_masks.iter_mut().enumerate() {
		*mask = rook_mask(i as u8);
	}
	let mut bishop_masks = [Bitboard8x8::default(); Bitboard8x8::NB_SQUARES];
	for (i, mask) in bishop_masks.iter_mut().enumerate() {
		*mask = bishop_mask(i as u8);
	}

	let rook_pext = Bitboard8x8::generate_attack_tables_pext(rook_mask, |square, blockers| {
		Bitboard8x8::compute_attacks(ROOK_OFFSETS, square, blockers)
	});
	let bishop_pext = Bitboard8x8::generate_attack_tables_pext(bishop_mask, |square, blockers| {
		Bitboard8x8::compute_attacks(BISHOP_OFFSETS, square, blockers)
	});

	writeln!(
		f,
		"pub static ROOK_MASKS: [Bitboard8x8; {}] = {:?};",
		Bitboard8x8::NB_SQUARES,
		rook_masks
	)
	.unwrap();
	writeln!(
		f,
		"pub static BISHOP_MASKS: [Bitboard8x8; {}] = {:?};",
		Bitboard8x8::NB_SQUARES,
		bishop_masks
	)
	.unwrap();
	...
	writeln!(
		f,
		"pub static ROOK_MOVES: [&[Bitboard8x8]; {}] = [",
		Bitboard8x8::NB_SQUARES
	)
	.unwrap();
	...
	writeln!(
		f,
		"pub static BISHOP_MOVES: [&[Bitboard8x8]; {}] = [",
		Bitboard8x8::NB_SQUARES
	)
	.unwrap();
	for sq in 0..Bitboard8x8::NB_SQUARES {
